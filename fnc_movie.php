<?php
	...
	function read_all_person_for_option($selected){
		...
		echo $conn->error;
		$stmt->bind_result($id_from_db, $first_name_from_db, $last_name_from_db, $birthdate_from_db);
		$stmt->execute();
		...
		while($stmt->fetch()){
			 ...
			 $options_html .= ">" .$first_name_from_db ." " .$last_name_from_db . " (" .$birthdate_from_db .")</option> \n";
		 }
		$stmt->close();
    ...
    function store_person_photo($file_name, $person_id){
		...
		if($stmt->execute()){
                $notice = "Uus pilt edukalt salvestatud!";
            } else {
                $notice = "Uue pildi salvestamisle tekkis viga: " .$stmt->error;
            }
